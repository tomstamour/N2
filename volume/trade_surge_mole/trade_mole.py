#!/usr/bin/env python3
"""
IBKR Trade-Frequency Surge Detector
====================================
...
----------------------
1. reqTickByTickData("AllLast")   -> every individual trade print (no aggregation)
2. reqTickByTickData("BidAsk")    -> every NBBO change
3. reqMktData generic ticks       -> 233, 236, 293, 294, 295, 318, 375, 165, 221

Rolling windows: 1s, 2s, 3s, 4s, 5s, 10s.
Baseline trade rate is supplied externally via --baseline-trade-per-second
(historical, pre-computed). Timing uses time.perf_counter() (monotonic,
microsecond resolution). Callback hot-path is minimized: O(n) window scan
over a bounded 10s deque.

Usage
-----
    python ibkr_trade_surge.py \\
        --symbol AAPL \\
        --clientID 7 \\
        --lifeTime 15:00 \\
        --output ./data/ \\
        --baseline-trade-per-second 0.05
...
class IBKRSurgeApp(EWrapper, EClient):
    ...
    *after* disconnect(), so no locking is required.
    ...
    Hot-path ordering in tickByTickAllLast:
      1) perf_counter timestamp
      2) append to trade deque + trim
      3) compute windows (O(n) over bounded deque)
      4) surge detection
      5) build record dict and append

    Steps 1-4 together run in single-digit microseconds on modern hardware
    for typical small-cap trade rates.
    """

    def __init__(self, symbol: str, baseline_trade_rate: float):
        EClient.__init__(self, wrapper=self)
        self.symbol = symbol
        self._hist_baseline_trade_rate = baseline_trade_rate
        self._hist_baseline_avg_iti = 1.0 / baseline_trade_rate

        # Request IDs
        self.req_id_mkt = 1001
        self.req_id_tbt_last = 1002
        self.req_id_tbt_bidask = 1003
        ...
        # Rolling trade log: list of (mono_ts, price, size) capped at DEQUE_HISTORY_SECONDS
    ...
    # ------------------------------------------------------------------
    # Trade events (tickByTickAllLast) - PRIMARY signal source
    # ------------------------------------------------------------------

    def tickByTickAllLast(
        self, reqId, tickType, time_epoch, price, size,
        tickAttribLast, exchange, specialConditions
    ):
        """Fires on every trade print (including odd lots). Hot path."""
        ...
        # Append + trim trade log to deque history window
        self._trade_log.append((now_mono, price, size))
        ...
        rec = {
            ...
            "exchange_time_epoch": time_epoch,
            "exchange_time_iso": datetime.fromtimestamp(time_epoch).isoformat() if time_epoch else None,
            "tick_type": tickType,
            "price": price,
            "size": size,
            "exchange": exchange,
            ...
            # Quote snapshot at time of trade
            "bid": self._last_bid,
            "ask": self._last_ask,
        ...
        if surge:
            logging.warning(
                ...
                f"| hist_baseline={win['hist_baseline_trade_rate']:.2f}/s "
                f"| iti={iti:.2f}s | {reason}"
            )
    ...
    def _compute_windows(self, now_mono: float) -> dict:
        ...
        for ts, price, sz in self._trade_log:
            age = now_mono - ts
            for w in WINDOWS:
                if age <= w:
                    counts[w] += 1
    ...
    def _detect_surge(self, w: dict, iti: Optional[float]) -> tuple:
        ...
        always defined, supplied via --baseline-trade-per-second).
        """
        reasons = []
    ...
    # ------------------------------------------------------------------
    # Quote events (tickByTickBidAsk)
    # ------------------------------------------------------------------

    def tickByTickBidAsk(
        self, reqId, time_epoch, bidPrice, askPrice, bidSize, askSize, tickAttribBidAsk
    ):
        now_wall = time.time()
        self._last_bid = bidPrice if bidPrice and bidPrice > 0 else None
        self._last_ask = askPrice if askPrice and askPrice > 0 else None
        self._last_bid_size = float(bidSize) if bidSize else None
        self._last_ask_size = float(askSize) if askSize else None
        ...
        self.records.append({
            ...
            "ask_size": self._last_ask_size,
            "spread": self._spread(),
            "spread_pct": self._spread_pct(self._midprice() or 0),
            "midprice": self._midprice(),
            "microprice": self._microprice(),
    ...
    def tickSize(self, reqId, tickType, size):
        ...
        if tickType == 0:
            ...
            self._tws_trade_count = int(sz)
            rec["tws_trade_count"] = int(sz)
        elif tickType == 89:
    ...
    def tickString(self, reqId, tickType, value):
        ...
        if tickType in (48, 77) and value:
            try:
                parts = value.split(";")
                if len(parts) >= 6:
                    rec["rt_price"] = float(parts[0]) if parts[0] else None
                    rec["rt_size"] = int(parts[1]) if parts[1] else 0
                    rec["rt_time_ms"] = int(parts[2]) if parts[2] else None
                    rec["rt_total_volume"] = int(parts[3]) if parts[3] else None
                    vwap = float(parts[4]) if parts[4] else None
                    rec["rt_vwap"] = vwap
                    rec["rt_single_trade_flag"] = (parts[5].lower() == "true")
                    rec["rt_source"] = "RTVolume" if tickType == 48 else "RTTradeVolume"
                    if vwap:
                        self._vwap = vwap
...
def main():
    parser = argparse.ArgumentParser(
        description="IBKR trade-frequency surge detector (Level-1 tick-by-tick)",
    ...
    parser.add_argument("--baseline-trade-per-second", dest="baseline_trade_rate",
                        type=float, required=True,
                        help="Externally computed historical baseline trade rate "
                             "(trades/sec, > 0). Used as the denominator for "
                             "surge-detection acceleration ratios and the implied "
                             "baseline avg inter-trade time (1/baseline).")
    ...
    if args.baseline_trade_rate <= 0:
        parser.error(
            f"--baseline-trade-per-second must be > 0, got {args.baseline_trade_rate}"
        )
    ...
    logging.info(
        f"Historical baseline: {args.baseline_trade_rate} trades/sec "
        f"(avg ITI {1.0/args.baseline_trade_rate:.2f}s)"
    )

    # --- Connect ---
    app = IBKRSurgeApp(symbol, baseline_trade_rate=args.baseline_trade_rate)
    ...
    try:
        logging.info(f"Requesting tickByTick AllLast for {symbol} (id={app.req_id_tbt_last})")
        app.reqTickByTickData(app.req_id_tbt_last, contract, "AllLast", 0, False)

        logging.info(f"Requesting tickByTick BidAsk for {symbol} (id={app.req_id_tbt_bidask})")
        app.reqTickByTickData(app.req_id_tbt_bidask, contract, "BidAsk", 0, True)
        ...
        try:
            app.cancelTickByTickData(app.req_id_tbt_last)
            app.cancelTickByTickData(app.req_id_tbt_bidask)
