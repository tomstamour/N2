#!/usr/bin/env python3
"""
IBKR Trade-Frequency Surge Detector (v4.3 — pool-worker variant)
================================================================

Drop-in successor to ``trade_mole_4.py`` that adds a single new CLI flag,
``--pool-mode``, used by ``Orchestrator3.7.py`` to keep 10 IBKR API
clients (clientIDs 330–339) pre-connected and idle. When a news trigger
fires, the orchestrator hands the symbol to an already-connected worker
over stdin, eliminating the 1–3 s connect-time latency that
``trade_mole_4.py`` paid on every fresh subprocess.

Two runtime modes
-----------------

1. **Single-symbol mode** (no ``--pool-mode``):
   Byte-compatible with ``trade_mole_4.py`` for ad-hoc CLI runs.

       python3 trade_mole_4.3.py --symbol AAPL --clientID 7 \\
           --lifeTime 15:00 --output ./data/ --baseline-iti 0.2

2. **Pool mode** (``--pool-mode``, used only by Orchestrator3.7):
       python3 trade_mole_4.3.py --pool-mode --clientID 330 --port 4001 \\
           --log-dir /path/to/tm_logs

   On startup:
     a. Connect to IBKR with ``--clientID``.
     b. Emit ``{"event":"ready","client_id":<N>}\\n`` on stdout, flush.
     c. Block on ``sys.stdin.readline()`` waiting for one JSON command:

            {"symbol":"AAPL","baseline_iti":44.4,"lifeTime_sec":600,
             "output_dir":"/.../trade_mole_outputs",
             "log_dir":"/.../trade_mole_logs"}

     d. Configure the existing ``IBKRSurgeApp`` with the supplied symbol
        and baseline, then run the standard subscribe → collect → teardown
        cycle exactly as ``trade_mole_4.py`` would.
     e. Exit cleanly when the lifeTime expires. The orchestrator's pool
        reaper detects exit and respawns a replacement worker.

   In pool mode stdout is reserved exclusively for the READY ack — every
   other log line is routed to ``--log-dir/pool_<clientID>_<ts>.log`` so
   the orchestrator's stdout reader never sees noise.

The ``IBKRSurgeApp`` class is unchanged from ``trade_mole_4.py``.
...
# Max time we'll wait for one stdin JSON command in pool mode. The pool
# reaper would not normally let a worker idle this long, but if stdin
# input never arrives (orchestrator hung) we exit so the slot can respawn.
POOL_STDIN_WAIT_TIMEOUT_SEC = 24 * 3600  # 24h — effectively "forever"
...
# ============================================================================
# IBKR Client / Wrapper  — UNCHANGED from trade_mole_4.py
# ============================================================================
...
class IBKRSurgeApp(EWrapper, EClient):
    ...
    *after* disconnect(), so no locking is required.
    ...
    Pool-mode note (v4.3): instances are now constructed with placeholder
    ``symbol`` / ``baseline_avg_iti`` values during the idle pre-trigger
    phase. ``main()`` overwrites ``self.symbol``, ``self._hist_baseline_avg_iti``
    and ``self._hist_baseline_trade_rate`` immediately after the stdin
    command is parsed, before reqMktData is issued — so no callback ever
    sees the placeholder values.
    """
...
def _open_pool_log(log_dir: str, client_id: int) -> str:
    ...
    One log file per worker process — covers both the idle phase and the
    active mole phase. Symbol is appended via a log message after stdin parse."""
    os.makedirs(log_dir, exist_ok=True)
    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    return os.path.join(log_dir, f"pool_{client_id}_{ts}.log")


def _parse_pool_command(line: str) -> dict:
    """Validate & parse the one-line JSON command the orchestrator sends.
    Raises ValueError on missing/invalid keys."""
    cmd = json.loads(line)
    required = ("symbol", "baseline_iti", "lifeTime_sec", "output_dir")
    missing = [k for k in required if k not in cmd]
    if missing:
        raise ValueError(f"pool command missing keys: {missing}")
    if float(cmd["baseline_iti"]) <= 0:
        raise ValueError(f"baseline_iti must be > 0, got {cmd['baseline_iti']}")
    if int(cmd["lifeTime_sec"]) <= 0:
        raise ValueError(f"lifeTime_sec must be > 0, got {cmd['lifeTime_sec']}")
    return cmd
...
def main():
    parser = argparse.ArgumentParser(
        description="IBKR trade-frequency surge detector — reqMktData-only, external baseline (ITI input). "
                    "v4.3 adds --pool-mode for pre-connected worker pools.",
    ...
    parser.add_argument("--pool-mode", action="store_true",
                        help="Run as a pre-connected pool worker controlled by "
                             "Orchestrator3.7.py. The worker connects to IBKR, "
                             "emits one READY line on stdout, then blocks reading "
                             "one JSON command from stdin before subscribing.")
    ...
    # =========================================================================
    # POOL MODE — connect first, idle on stdin, then run one mole and exit
    # =========================================================================
    if args.pool_mode:
        ...
        # Both are overwritten before reqMktData is issued.
        ...
        # READY ack — single line of stdout, exclusively used by the
        # orchestrator's reader thread. Flush so Popen sees it immediately.
        ready_msg = {"event": "ready", "client_id": args.clientID}
        print(json.dumps(ready_msg), flush=True)
        logging.info(f"[pool] READY emitted; awaiting trigger on stdin")

        # Block reading one JSON line. EOF (orchestrator dies) → clean exit.
        try:
            line = sys.stdin.readline()
        except KeyboardInterrupt:
            logging.info("[pool] interrupted while idle. Disconnecting.")
        ...
        try:
            cmd = _parse_pool_command(line.strip())
        except (ValueError, json.JSONDecodeError) as e:
            logging.error(f"[pool] invalid stdin command {line!r}: {e}")
            app.disconnect(); api_thread.join(timeout=5)
            sys.exit(2)

        # Bind the real symbol / baseline onto the existing app instance.
        # Safe to mutate from main thread: no reqMktData has been issued yet,
        # so the ibapi callback thread can only have produced infra messages
        # (nextValidId, connection acks) — none of which read these fields.
        app.symbol = cmd["symbol"].upper()
        app._hist_baseline_avg_iti = float(cmd["baseline_iti"])
        app._hist_baseline_trade_rate = 1.0 / app._hist_baseline_avg_iti
        lifeTime_sec = int(cmd["lifeTime_sec"])
        output_path = resolve_output_path(cmd["output_dir"], app.symbol)

        logging.info(
            f"[pool] trigger received: symbol={app.symbol} "
            f"baseline_iti={app._hist_baseline_avg_iti:.2f}s "
            f"lifeTime={lifeTime_sec}s output={output_path}"
        )

        # Run the standard subscribe → collect → teardown cycle.
        _run_subscription_cycle(app, api_thread, lifeTime_sec, output_path)
        return

    # =========================================================================
    # SINGLE-SYMBOL MODE — byte-compatible with trade_mole_4.py
    # =========================================================================
...
def _run_subscription_cycle(
    ...
    """Shared between pool-mode and single-symbol mode: pre-04:00 ET gate,
    reqMktData subscription, lifeTime collection loop, teardown, CSV write.

    Carved out of the original trade_mole_4.py main() so both entry paths can
    reuse the same orchestrated sequence. Behavior is preserved byte-for-byte
    from trade_mole_4.py (subscribe → 30s progress logs → cancel → disconnect
    → DataFrame build → CSV write)."""
    # --- Pre-04:00 ET gate ---
    # In pool mode this is normally a no-op (Orchestrator3.7 already defers
    # off-hours triggers via threading.Timer). Kept as belt-and-suspenders.
