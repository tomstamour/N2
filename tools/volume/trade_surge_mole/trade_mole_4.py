#!/usr/bin/env python3
"""
IBKR Trade-Frequency Surge Detector (external-baseline, reqMktData-only)
=========================================================================
...
This variant combines:
  * The low API-cost design of trade_mole_3.py — reqMktData only (no
    reqTickByTickData), so each process consumes one market-data line
    (~100/account default) instead of two tick-by-tick streams (3/account
    hard cap). Spawn one process per symbol with a unique --clientID to
    track many symbols simultaneously.
  * The external-baseline surge detection of trade_mole.py — three rules
    that compare short-window rates against an externally supplied
    historical baseline trade rate (constant for the session).

The CLI input unit for the baseline is **average inter-trade interval
(ITI) in seconds**, supplied via --baseline-iti. The implied baseline
trade rate is 1/baseline-iti. ITI is the more natural way to describe
quiet pre-market microcap regimes (e.g. one trade every 60s).
...
Usage
-----
    # Liquid large-cap (baseline ~5 trades/s -> avg ITI ~0.2s)
    python trade_mole_4.py \\
        --symbol AAPL \\
        --clientID 7 \\
        --lifeTime 15:00 \\
        --output ./data/ \\
    ...
    # Illiquid pre-market microcap (one trade every ~60s)
    python trade_mole_4.py \\
        --symbol SOFI \\
        --clientID 3 \\
        --lifeTime 05:00 \\
        --output /tmp/SOFI_premarket.txt \\
        --port 4001 \\
        --baseline-iti 60.0
        ...
        if tickType in (48, 77) and value:
            try:
                ...
                if len(parts) >= 6 and parts[0] and parts[1]:
                    ...
                    # IBKR formats size/total_volume as float strings (e.g. "5.0000000000000000");
                    # int() on those raises ValueError, so go through float() first.
...
def main():
    ...
    parser.add_argument("--baseline-iti", dest="baseline_avg_iti",
                        ...
                        help="Externally computed historical baseline average "
                             ...
                             "Example: --baseline-iti 20.0 means one trade every "
                             "20s on average (rate = 0.05 trades/s).")
