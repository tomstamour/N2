...
End-to-end pipeline:
  ...
  3. Append RTH_avgITI_sec / ETH_avgITI_sec via trade_frequency_addOn
  4. Save to ./data/nasdaq_symbols_data_priced_YYYY-MM-DD.tsv
     (manual `--limit N` runs are written to
     ./data/nasdaq_symbols_data_priced_YYYY-MM-DD_limitN.tsv so a small test
     never silently overwrites the same-day nightly cron output.)
  5. Run ITI_imputer.py on the file just written, OVERWRITING it
     in-place with the 12-column imputed version (4 sidecar columns
     added: imputed RTH/ETH values, status flag, method tag). The
     canonical RTH/ETH columns carry NaN where the input had the 44444
     sentinel; the sidecars carry the model predictions. Skipped by
     `--no-impute`. Imputer failure does NOT clobber the canonical
     8-column file — the write at step 4 is preserved on any imputer
     exception (atomic temp+rename inside ITI_imputer guarantees this).

  Output columns (8 base; 12 after ITI_imputer adds its 4 sidecars):
...
Scheduled daily at 20:30 ET by cron (see crontab entry at bottom of file).

Usage:
    python pipeline_daily.py [--host HOST] [--port PORT] [--clientID ID]
...
def main() -> None:
    ...
    from trade_frequency_addOn import fetch_freq_async, past_trading_days  # noqa: E402
    ...
    try:
        ...
        # fetch_freq_async returns a 2-tuple (rth_iti, eth_iti).
    ...
    output_path = _DATA_DIR / f"nasdaq_symbols_data_priced_{date_tag}{limit_tag}.tsv"
    ...
    if not limit_tag and output_path.exists():
        ...
        if valid_rth >= existing_rth and valid_eth >= existing_eth:
            ...
            superseded_path = _DATA_DIR / (
                f"nasdaq_symbols_data_priced_{date_tag}_{ts}_superseded.tsv")
            try:
            ...
            ts = datetime.now().strftime("%H%M")
            sidecar = _DATA_DIR / (
                f"nasdaq_symbols_data_priced_{date_tag}_{ts}.tsv")
            log.warning(
    ...
    # overwrite it in-place with the 12-column imputed version (4 sidecar
    # columns added; canonical RTH/ETH cols keep NaN where 44444 was).
    # ITI_imputer's write is atomic (temp + os.replace), so a crash
    # mid-write cannot corrupt the canonical file. Imports are deferred
    # (sklearn is heavy) so `--no-impute` and pre-impute failures stay
    # zero-cost.
    #
    # Runs on whatever `output_path` was actually written, including:
    #   - canonical `..._YYYY-MM-DD.tsv`  (cron path)
    #   - overwrite-protected `..._YYYY-MM-DD_HHMM.tsv` sidecar
    #   - manual `..._YYYY-MM-DD_limitN.tsv` test runs (ITI_imputer's
    ...
    if args.no_impute:
        ...
        try:
            ...
            if rc == 0:
                log.info("  ITI_imputer: ok (12-col file written in place).")
            else:
            ...
            log.exception(f"ITI_imputer failed: {exc!r} — 8-col canonical file "
                          f"is unaffected; imputation can be re-run manually.")
...
# ─── Cron entry (add once with: crontab -e) ───────────────────────────────────
#
# TZ=America/New_York
...
#
# `runs/cron.log` is bounded by `_rotate_cron_log()` at the top of every run:
# once it exceeds 5 MB it is renamed to `cron.log.1` (and `cron.log.1` to
# `cron.log.2`), so the active file always starts each night ≤ 5 MB. The
# per-day `runs/{DD-MMM-YYYY}/pipeline_daily.log` keeps the full run record.
