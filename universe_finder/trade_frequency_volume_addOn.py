#!/usr/bin/env python3
"""
trade_frequency_volume_addOn.py
===============================
...
*same* fetched TRADES bars (no extra IBKR request), the average volume per
trade for the RTH and ETH sessions.
...
Four columns are appended:
  ...
  RTH_volPerTrade  — avg shares per trade, RTH session, over the lookback window
  ETH_volPerTrade  — avg shares per trade, ETH sessions, over the lookback window
...
Volume per trade is computed as an *aggregate ratio* over the whole lookback
...
the volume metric — the aggregate ratio is the intended statistic.

Usage
-----
    python trade_frequency_volume_addOn.py \\
...
# per-session volume sums used by RTH_volPerTrade / ETH_volPerTrade.
_FREQ_BAR_SIZE = "30 mins"

# IBKR reqHistoricalData TRADES volume for US stocks is reported in round lots
# of 100. Multiply to express RTH_volPerTrade / ETH_volPerTrade in actual
# shares, matching trade_mole_4.1.py's RTVolume-based mean_size_{w}s (which is
# in real shares). NOTE: verify empirically (reconstruct one liquid symbol's
# daily volume and compare to a known figure) — set to 1 if this feed already
# returns actual shares.
_IBKR_VOLUME_LOT_MULTIPLIER = 100
...
def main():
    parser = argparse.ArgumentParser(
        description="Enrich a symbol TSV with avg inter-trade interval and avg "
                    "volume-per-trade for RTH and ETH sessions.",
    ...
    df["RTH_volPerTrade"] = rth_vol_results
    df["ETH_volPerTrade"] = eth_vol_results
    df["RTH_avgITI_sec"]  = df["RTH_avgITI_sec"].fillna(44444)
    df["ETH_avgITI_sec"]  = df["ETH_avgITI_sec"].fillna(44444)
    df["RTH_volPerTrade"] = df["RTH_volPerTrade"].fillna(44444)
    df["ETH_volPerTrade"] = df["ETH_volPerTrade"].fillna(44444)

    valid_rth     = sum(1 for v in rth_results if v == v)
    valid_eth     = sum(1 for v in eth_results if v == v)
    valid_rth_vol = sum(1 for v in rth_vol_results if v == v)
    valid_eth_vol = sum(1 for v in eth_vol_results if v == v)
    log.info(
        f"RTH_avgITI_sec: {valid_rth}/{total} populated  "
        f"ETH_avgITI_sec: {valid_eth}/{total} populated  "
        f"RTH_volPerTrade: {valid_rth_vol}/{total} populated  "
        f"ETH_volPerTrade: {valid_eth_vol}/{total} populated"
    )
