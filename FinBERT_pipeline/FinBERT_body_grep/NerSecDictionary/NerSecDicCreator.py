...
class TickerResolver:
    ...
    def _build_alias_map(self):
        ...
        for cik, company in self.sec_tickers.items():
            ...
            # Add bare single-word name by stripping normalized suffix forms.
            # SEC EDGAR names are all-caps (e.g. "BROADWIND, INC."), so the
            # title-case suffix loop above misses them. Stripping lowercase
            # normalized suffixes from `normalized_title` covers that gap.
            _BARE_NAME_SKIP = frozenset({
                'inc', 'corp', 'ltd', 'llc', 'co', 'company', 'holdings',
                'group', 'limited', 'incorporated', 'corporation',
            })
            _NORM_SUFFIXES_BARE = (
                ' inc', ' corp', ' ltd', ' llc', ' co', ' company',
                ' holdings', ' group', ' plc', ' sa', ' ag', ' ab', ' nv',
            )
            for ns in _NORM_SUFFIXES_BARE:
                if normalized_title.endswith(ns):
                    bare = normalized_title[:-len(ns)].strip()
                    if bare and len(bare) > 3 and ' ' not in bare and bare not in _BARE_NAME_SKIP:
                        self.aliases.setdefault(bare, ticker)
                    break
...
class EntityExtractor:
    ...
    # Single capitalized word ≥7 chars, starting Xx (excludes all-caps abbreviations).
    # Catches bare company names like "Broadwind" that appear without a corporate suffix.
    _SINGLE_CAP_WORD_PATTERN = re.compile(r'\b([A-Z][a-z][a-zA-Z]{5,})\b')
    ...
    def extract_entities(text: str) -> List[Tuple[str, int, int]]:
        ...
        # Extract single capitalized words (≥7 chars) as potential bare company names.
        # Unresolved words become match_type="unresolved" and are ignored downstream.
        for match in EntityExtractor._SINGLE_CAP_WORD_PATTERN.finditer(text):
            word = match.group()
            if (word not in seen
                    and word not in EntityExtractor.FALSE_POSITIVES
                    and word not in EntityExtractor._SENTENCE_INITIAL_WORDS):
                entities.append((word, match.start(), match.end()))
                seen[word] = (match.start(), match.end())
