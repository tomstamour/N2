...
class EntityExtractor:
    ...
    # Single mixed-case capitalized word ≥4 chars (catches bare names like "Broadwind").
    # Requires second char lowercase to exclude ALL-CAPS acronyms already handled above.
    _SINGLE_CAP_WORD_PATTERN = re.compile(r'\b([A-Z][a-z][a-zA-Z]{2,})\b')
    ...
    def extract_entities(text: str) -> List[Tuple[str, int, int]]:
        ...
        # Strategy D: Single capitalized words that may be bare company names.
        # e.g. "Broadwind generated…" → extracts "Broadwind" → alias map → BWEN.
        # Mixed-case requirement (upper + lower) excludes ALL-CAPS acronyms already
        # handled by the ticker strategy. Non-company words (e.g. "Illinois", "Eric")
        # are harmlessly returned as match_type="unresolved" by TickerResolver.
        for match in EntityExtractor._SINGLE_CAP_WORD_PATTERN.finditer(text):
            ...
            if (word not in seen
                ...
                entities.append((word, match.start(1), match.end(1)))
                seen[word] = (match.start(1), match.end(1))
