#!/usr/bin/env python3
"""
FinBERT_body_withCoref.py - FinBERT sentiment pipeline with full coreference
...
logger = logging.getLogger("FinBERT_body_withCoref")
...
class FinBERTBodyPipeline:
    ...
    Requires fastcoref. Will raise RuntimeError at load_models() if not installed.
    ...
    def load_models(self) -> None:
        ...
        t0 = time.time()
        logger.info("Loading models (full coref variant)...")
...
def main():
    parser = argparse.ArgumentParser(
        description="FinBERT body pipeline (full coref): clean -> corefs -> split -> NER -> FinBERT"
    )
