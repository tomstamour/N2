#!/usr/bin/env python3
"""
FinBERT-analysis.py — Entity-targeted financial sentiment analysis (ONNX INT8)

Runs ProsusAI/finbert on NER-annotated JSON input produced by NerSecDicCreator.
...
Two-phase usage:
    ...
    python FinBERT-analysis.py --export

    # Phase 2 — fast inference (loads pre-exported model, processes input file)
    python FinBERT-analysis.py --input ./FEED_28-jan-2026_sentences_NER.json
    python FinBERT-analysis.py --input ./FEED_28-jan-2026_sentences_NER.json --output ./results.json
"""
...
class FinBERTProcessor:
    ...
    def run(self) -> bool:
        ...
        if not self.model_dir.exists():
            logger.error(
                ...
                f"  python FinBERT-analysis.py --export"
            )
            return False
    ...
    def _derive_output_path(input_path: Path) -> Path:
        """Default: {stem}_FinBERT.json in the same directory as the input file."""
        return input_path.parent / (input_path.stem + "_FinBERT.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Entity-targeted FinBERT sentiment analysis (ONNX INT8)",
...
Examples:
  ...
  python FinBERT-analysis.py --input ./data.json --output ./results.json
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output JSON path (default: <input_stem>_FinBERT.json)",
    )
