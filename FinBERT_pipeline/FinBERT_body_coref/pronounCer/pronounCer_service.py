...
def _find_we_pronouns_for_global_entity(text, global_main_entity, existing_replacements):
    """
    Within quoted regions (straight or curly quotes), find any "we"/"We" tokens
    not already scheduled for replacement and map them to global_main_entity.

    Unlike find_pronouns_in_quotes(), this does NOT skip tokens that are in a
    fastcoref cluster — it only skips tokens already being replaced.  This is
    necessary because fastcoref sometimes places "we" in a separate cluster with
    no entity canonical, leaving it unreplaced even when "our" in the same
    sentence was correctly resolved to the company name.
    """
    if not global_main_entity:
        return []
    ...
    # Support both straight "..." and curly “...” quote delimiters.
    quote_pattern = re.compile(r'["“]([^"”]+)["”]')
    we_pattern    = re.compile(r'\b[Ww]e\b')

    for q_match in quote_pattern.finditer(text):
        q_start = q_match.start() + 1   # skip opening quote char (1 byte)
        for m in we_pattern.finditer(q_match.group(1)):
            abs_start = q_start + m.start()
            abs_end   = q_start + m.end()
            if not any(i in replacement_ranges for i in range(abs_start, abs_end)):
                additional.append((abs_start, abs_end, global_main_entity))
    ...
    def resolve_text(self, text):
        ...
        try:
            ...
            if global_candidates:
                ...
                # Fallback: no suffixed candidate found (company name used without
                # "Inc."/"Corp." etc.). Pick the shortest capitalized, non-pronoun,
                # non-generic mention from any large cluster (≥4 members).
                _PRONOUNS = frozenset({'we', 'our', 'us', 'it', 'he', 'she', 'they',
                                       'their', 'i', 'you', 'your', 'its'})
                fallback_candidates = []
                for cluster in clusters:
                    if len(cluster) < 4:
                        continue
                    for m_start, m_end in cluster:
                        mention = text[m_start:m_end].strip()
                        if (mention
                                and mention[0].isupper()
                                and len(mention) >= 4
                                and mention.lower() not in _PRONOUNS
                                and not is_purely_generic_corporate(mention)):
                            fallback_candidates.append(mention)
                if fallback_candidates:
                    global_main_entity = min(
                        fallback_candidates,
                        key=lambda m: (any(c in m for c in _NOISE_CHARS), len(m)),
                    )
                    logger.debug(f"=== Global main entity (fallback): '{global_main_entity}' ===")
                else:
            ...
            for i, cluster in enumerate(clusters):
                ...
                for mention_span in cluster[1:]:
                    ...
                    if len(mention_text) <= 2 and mention_text.lower() in {'it', 'he', 'i', 'the', 'a'}:
                        continue
            ...
            # Step 6.6: Replace unreplaced "we"/"We" inside quotes with global entity.
            # Handles cases where fastcoref clustered "we" separately from the company,
            # leaving it unreplaced even after all cluster and pronoun passes.
            we_replacements = _find_we_pronouns_for_global_entity(
                text, global_main_entity, existing_replacements=replacements
            )
            if we_replacements:
                logger.debug(f"Found {len(we_replacements)} 'we'/'We' pronouns for global entity:")
                for s, e, repl in we_replacements:
                    logger.debug(f"  ({s}-{e}): '{text[s:e]}' -> '{repl}'")
            replacements.extend(we_replacements)
