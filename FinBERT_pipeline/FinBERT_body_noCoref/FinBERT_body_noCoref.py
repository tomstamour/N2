#!/usr/bin/env python3
"""
FinBERT_body_noCoref.py - Fast FinBERT sentiment pipeline for the
`article_body` field of news JSON files. Coreference resolution is
intentionally omitted for speed.

Pipeline (in order):
    1. jsonCleaner       - HTML/entity/whitespace/URL/ticker scrub
    2. SentenceSplitter  - sentence boundary detection (spaCy)
    3. NerSecDicCreator  - SEC-EDGAR ticker resolution NER
    4. FinBERT-analysis  - entity-targeted FinBERT (ONNX INT8) sentiment

Use FinBERT_body_pipeline.py when coreference resolution is required.
"""
...
_FINBERT_DIR     = _THIS_DIR.parent.parent / "FinBERT"
...
logger = logging.getLogger("FinBERT_body_noCoref")
...
class FinBERTBodyPipeline:
    """End-to-end pipeline (no coreference) for sentiment analysis of an article_body field.
    ...
    def load_models(self) -> None:
        ...
        t0 = time.time()
        logger.info("Loading models (no-coref variant)...")
    ...
    def process(
        ...
        """Run the pipeline (no coreference) on one article.
    ...
    def _step_split(self, text: str, source_file: str,
        ...
        metadata = {
            "input_file":       f"{source_file}_cleaned.json",
    ...
    def _step_finbert(self, ner_block: Dict, source_file: str,
        ...
        for sentence in sentences:
            ...
            if not deduped:
                # NER found no resolved entities. Fall back to the article's known
                # allowed_tickers (full-text scoring, no substitution) so that tickers
                # absent from the SEC EDGAR cache still receive sentiment scores.
...
def main():
    parser = argparse.ArgumentParser(
        description="FinBERT body pipeline (no coreference): clean -> split -> NER -> FinBERT"
    )
