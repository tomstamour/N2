...
TM_TRADE_MOLE_SCRIPT  = '/home/tom/Documents/ibkr_scripts/N2/scripts/volume/trade_surge_mole/trade_mole_4.3.py'
...
logger = logging.getLogger('Orchestrator3.7')
...
# (slot.dispatch() reads _lookup_baseline_iti which needs _iti_df).
_pool: Optional["TradeMolePool"] = None


class _PoolSlot:
    """One pre-connected worker. State machine:

       spawning → connecting → idle → running → dead → (respawn)

    Reader thread reads stdout: the first {"event":"ready",...} line
    transitions the slot from 'connecting' → 'idle' and pushes self onto
    the pool's _idle_queue. Reaper thread blocks on proc.wait(); on exit
    it marks 'dead' and (unless pool is shutting down) spawns a fresh
    worker on the same clientID after an exponential backoff if the
    previous worker failed before reaching 'idle'.
    """
    ...
    def __init__(self, client_id: int, pool: "TradeMolePool"):
        ...
        self.state: str = 'spawning'   # spawning|connecting|idle|running|dead
    ...
    def _reader_thread(self) -> None:
        """Drains worker stdout line-by-line. The READY ack transitions
        'connecting' → 'idle' and enqueues this slot. Subsequent stdout
        lines (which the worker shouldn't emit in pool mode — its logs
        go to a file) are logged at DEBUG. EOF means the worker exited;
        the reaper handles state transitions."""
        proc = self.proc
            ...
            for line in proc.stdout:
                ...
                if msg.get('event') == 'ready':
                    ...
                    if ready:
                        ...
                        logger.info(
                            f"[TM] pool slot {self.client_id} ready "
    ...
    def _reaper_thread(self) -> None:
        ...
        with self.state_lock:
            ...
            if prev_state == 'connecting':
                self.consecutive_failures += 1
            elif prev_state in ('idle', 'running'):
    ...
    def dispatch(self, symbol: str, baseline_iti: float,
                 completed_dict: dict) -> bool:
        """Send the one-shot JSON command to this worker's stdin. Returns
        True on success, False if the slot was no longer idle (race with
        reaper) or the write failed."""
        with self.state_lock:
        ...
        logger.info(
            f"[TM] dispatched {symbol} → slot {self.client_id} pid={self.proc.pid} "
...
class TradeMolePool:
    ...
    def dispatch(self, completed_dict: dict) -> None:
        """Acquire an idle slot and send it the JSON command. If the pool
        is empty, drop the trigger and log a warning (user-chosen policy)."""
        ...
        # Drain dead slots from the queue front (they could have been put
        # there at READY but died before dispatch — defensive).
        slot: Optional[_PoolSlot] = None
        while True:
            try:
            ...
            with candidate.state_lock:
                if candidate.state == 'idle':
                    slot = candidate
                    break
            # State != idle (probably went dead) — drop and continue draining.
            logger.debug(
                f"[TM] dispatch: skipping slot {candidate.client_id} "
        ...
        if not slot.dispatch(symbol, baseline_iti, completed_dict):
            logger.warning(
                f"[TM] {symbol}: slot {slot.client_id} dispatch failed (race) — dropping trigger"
            )
            # Reaper will detect death and respawn.

    def shutdown(self) -> None:
        """Signal reapers to stop respawning; close stdin on idle workers
        so they exit on EOF. Running workers are left alone — they're
        detached (start_new_session=True) and will complete their lifeTime
        as orphans, writing their CSVs as usual."""
        self._shutdown_event.set()
        for slot in self.slots:
            with slot.state_lock:
                state = slot.state
                proc = slot.proc
            if state == 'idle' and proc is not None and proc.stdin is not None:
                try:
                    proc.stdin.close()
                    logger.info(f"[TM] slot {slot.client_id}: stdin closed (idle worker will exit on EOF)")
                except Exception as exc:
                    logger.debug(f"[TM] slot {slot.client_id}: stdin close raised: {exc}")
            elif state == 'running':
                logger.info(
                    f"[TM] slot {slot.client_id}: still running {slot.current_symbol} "
...
def main():
    global _pool
    logger.info("Orchestrator3.7 starting...")
    ...
    # Universe TSV first — _PoolSlot.dispatch() uses _lookup_baseline_iti.
    _load_latest_iti_tsv()
    ...
    # Register AFTER nw.start() so Orchestrator3.7's handlers override NW4's
    ...
    try:
        ...
        nw.stop()
        logger.info("Orchestrator3.7 stopped.")
