...
logger = logging.getLogger('Orchestrator3.1')
...
def on_news_accepted(news_dict: dict) -> None:
    ...
    is shared by every ticker's _collect_and_log task.
    """
    ...
    for symbol in tickers:
        _collect_executor.submit(_collect_and_log, news_dict, tickers, symbol, f_finbert)


# ─── Main ─────────────────────────────────────────────────────────────────────

def main():
    logger.info("Orchestrator3.1 starting...")
    ...
    # Register AFTER nw.start() so Orchestrator3.1's handlers override NW3's
    ...
    try:
        ...
        nw.stop()
        logger.info("Orchestrator3.1 stopped.")
