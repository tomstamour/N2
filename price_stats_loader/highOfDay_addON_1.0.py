...
def get_target_dates(arrival_time_str: str) -> list:
    """
    Return the trading date(s) to fetch for a given UTC-4 arrival time.
    Before market close (< 16:00 ET): returns [same_day].
    After market close (>= 16:00 ET): returns [same_day, next_trading_day].
    """
    dt = datetime.fromisoformat(arrival_time_str).replace(tzinfo=timezone.utc)
    same_day = dt.date()
    while same_day.weekday() >= 5:
        same_day += timedelta(days=1)
    if dt.hour < MARKET_CLOSE_ET_HOUR:
        return [same_day]
    else:
        return [same_day, next_trading_day(same_day)]
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            'and previous close for each symbol on the relevant trading day(s).\n\n'
            'For news arriving before market close (< 16:00 ET), fetches the same '
            'day\'s high. For after-market arrivals (>= 16:00 ET), fetches both the '
            'current day and the next trading day, and outputs whichever produced '
            'the higher DailyHigh(%%).\n\n'
            'Skips non-US exchange symbols automatically.\n\n'
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                target_dates = get_target_dates(arrival_str)
            except Exception as e:
            ...
            df.at[idx, 'DailyHigh($)'] = best_high
            df.at[idx, 'DailyHigh(%)'] = best_pct
